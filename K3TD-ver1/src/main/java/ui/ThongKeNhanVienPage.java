 ...
 * @author LENOVO
 */
public class ThongKeNhanVienPage extends javax.swing.JPanel {

    /**
     * Creates new form ThongKeHangHoaPage
     */
    public ThongKeNhanVienPage() {
        initComponents();
        config();
    }
    ...
    private void addTable() {
    	...
    	dsNV = new HoaDonDao().getDSNVbyHD(tuNgay, denNgay);
    	List<HoaDon> dsNVT = new PhieuTraHangDao().getDSNVbyHD(tuNgay, denNgay);
		// TODO Auto-generated method stub
    	int stt = 1;
		for (HoaDon hd : dsNV) {
			NhanVien nv = hd.getNhanVien();
			tableModel.addRow(new Object[] {stt++,nv.getMaNhanVien(),nv.getTenNhanVien(),Formater.decimalFormat(hd.getDiemQuyDoi()),Formater.decimalFormat(hd.getTongTien()),0,0,Formater.decimalFormat(hd.getTongTien())});			
			if(dsNVT.size()!=0) {
				for (HoaDon hoadon : dsNVT) {
					if(nv.getMaNhanVien().equals(hoadon.getNhanVien().getMaNhanVien())) {
    ...
    private DefaultPieDataset createPieDataset() {
        ...
        for(int i = 0;i<dsNV.size();i++) {
        	if(i<5) {
        		dataset.setValue(dsNV.get(i).getNhanVien().getTenNhanVien()+" ", dsNV.get(i).getDiemQuyDoi());
        	}else
        		slKhac+=dsNV.get(i).getDiemQuyDoi();
        }
        if(slKhac!=0)
    ...
    private JFreeChart createPieChart(DefaultPieDataset dataset) {
        JFreeChart chart = ChartFactory.createPieChart(
                "Top 5 nhân viên có nhiều hóa đơn nhất",
                dataset,
                true,
                true,
                false
    ...
    private CategoryDataset createCategoryDataset() {
        ...
        for(int i = 0;i<5;i++) {
        	if(tableModel.getRowCount()<i+1)
        		return dataset;
        	dataset.addValue(Double.parseDouble(tableModel.getValueAt(i, 7).toString().replaceAll(",", "").toString()), dsNV.get(i).getNhanVien().getTenNhanVien(), "Top "+(i+1));
        }
    ...
    private void initComponents() {
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã hàng hóa", "Tên hàng hóa", "Đơn vị tính", "Số lượng bán", "Số lượng trả", "Doanh thu"
            }
        ) {
