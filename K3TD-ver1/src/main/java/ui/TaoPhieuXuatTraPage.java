...
public class TaoPhieuXuatTraPage extends javax.swing.JPanel {
	...
	private DefaultTableModel model_HH;
	private JTable tbl_HH;
	...
	private void loadPXT() {
		// TODO Auto-generated method stub
		List<PhieuXuatTra> dsPNH = phieuXuatTra_dao.getAllDataPXT();
		dsPNH.forEach(pnh -> cb_maPhieuNhapMau.addItem(pnh.getMaPhieuXuatTra()));
	}
	...
	private void addNhapHangTable() {
		// TODO Auto-generated method stub
		String[] colNames = { "STT", "Mã HH", "Tên HH", "Số lô", "Ngày sản xuất", "Hạn sử dụng", "DVT", "Số lượng",
				"Giá nhập", "VAT (%)", "Chiết khấu", "Thành tiền", ""};
...
//		setCellEditable();
		...
		tbl_HH.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				// TODO Auto-generated method stub
				int row = tbl_HH.getSelectedRow();
				doDuLieuVaoComboBoxDonViTinh(model_HH.getValueAt(row, 1).toString());
			}
		});
		// su kien thay doi data table
					model_HH.addTableModelListener(new TableModelListener() {
						...
						public void tableChanged(TableModelEvent e) {
							...
							if (isUpdating) {
								return; // Nếu đang cập nhật, không làm gì cả
							}
	...
	public void doDuLieuVaoComboBoxDonViTinh(String maHH) {
    	cb_tenDVT = new JComboBox<>();
		List<DonViTinh> dsTenDVT = donViTinh_dao.timDVTTheoMaHH(maHH);
    ...
    private void initComponents() {
        ...
        cb_maPhieuNhapMau.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Chọn phiếu xuất mẫu --" }));
    ...
    private void cb_maPhieuNhapMauActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cb_maPhieuNhapMauActionPerformed
    	...
    	PhieuXuatTra pnh = phieuXuatTra_dao.getPXTTheoMa(ma);
    	List<ChiTietPhieuXuatTra> dsChiTiet= chiTietPXT_dao.getChiTietPXT(ma);
    	
    	txt_maPhieuNhap.setText(pnh.getMaPhieuXuatTra());
...
//    			double thanhTien = hh.getThue()*
				...
				if (!dataExist) {
					// "STT", "Mã HH", "Tên HH", "Số lô", "Hạn sử dụng", "DVT", "Số lượng", "Giá
					// nhập", "VAT (%)", "Chiết khấu", "Thành tiền"
					model_HH.addRow(new Object[] { rowCount + 1, dvt.getHangHoa().getMaHangHoa(),
							dvt.getHangHoa().getTenHangHoa(), "", "", "", dvt.getTenDonViTinh(), 1, 0,
							dvt.getHangHoa().getThue(), 0, 0 });
					txt_timHangHoa.setText("");
					txt_timHangHoa.requestFocus();
				}
			} else {
...
//				tbl_HH.getColumnModel().getColumn(6).setCellEditor(new DefaultCellEditor(cb_tenDVT));
				...
				model_HH.addRow(new Object[] { rowCount + 1, hh.getMaHangHoa(), hh.getTenHangHoa(), "", "", "", tenDVT,
						1, 0, hh.getThue(), 0, 0 });
...
//				tbl_HH.getColumnModel().getColumn(5).setCellEditor(new DatePickerEditor());
	...
	private void btn_taoPhieuNhap1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn_taoPhieuNhap1ActionPerformed
		...
		if (validData()) {
	    	...
	    	TrangThaiPhieuXuatTra trangThai = TrangThaiPhieuXuatTra.HOAN_THANH;
	    	
	    	PhieuXuatTra pxt = new PhieuXuatTra("null", tgTao, ghiChu, maHDNCC, ncc, tongGiamGia, tongTienHang, trangThai);
	    	System.out.println(pxt.toString());
	    	PhieuXuatTra pnHang = phieuXuatTra_dao.createPXT(pxt);
	    	...
	    	if (pnHang != null) {
	    		...
	    		for (int i = 0; i < tbl_HH.getRowCount(); i++) {
	    			...
	    			double donGia = Double.parseDouble(model_HH.getValueAt(i, 8).toString());
